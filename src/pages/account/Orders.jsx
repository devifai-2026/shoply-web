  ...
  return (
    <div className="bg-surface border border-border-minimal animate-in fade-in duration-700">
      <div className="flex items-center justify-between p-10 border-b border-border-minimal">
        <h2 className="text-[14px] font-normal text-ink uppercase tracking-[0.011em]">Transaction Registry</h2>
        <div className="flex gap-4">
          {['All', 'Active', 'History'].map(t => (
            <button
              ...
              onClick={() => setTab(t)}
              className={cn(
                'text-[11px] font-normal uppercase tracking-[0.011em] px-4 py-2 transition-colors',
                t === tab ? 'text-ink bg-bg' : 'text-subtle hover:text-ink'
              )}
            >
              {t}
            </button>
      ...
      <div className="divide-y divide-border-minimal">
        {filtered.map(order => (
          <Link
            ...
            className="p-10 flex flex-col md:flex-row md:items-center justify-between gap-10 hover:bg-bg transition-colors group"
          >
            <div className="flex items-center gap-10">
              <div className="w-24 h-24 bg-bg border border-border-minimal flex items-center justify-center shrink-0 group-hover:border-ink transition-colors">
                <Package className="w-8 h-8 text-subtle stroke-[1.2]" />
              </div>
              <div>
                <div className="flex items-center gap-4 mb-3">
                  <span className="text-[14px] font-medium text-ink uppercase tracking-tight">{order.orderNumber}</span>
                  <span className={cn(
                    'text-[10px] font-normal uppercase tracking-[0.011em] px-3 py-1 border border-border-minimal',
            ...
            <div className="flex items-center justify-end gap-8 w-full md:w-auto">
              <div className="text-right">
                <p className="text-[10px] font-normal uppercase tracking-[0.011em] text-subtle mb-1">Total</p>
                <p className="text-[20px] font-medium text-ink tracking-tight">{formatPrice(order.total)}</p>
              </div>
      ...
      {filtered.length === 0 && (
        <div className="py-24 text-center">
          ...
          <h3 className="text-[18px] font-normal text-ink mb-3">No activity recorded</h3>
